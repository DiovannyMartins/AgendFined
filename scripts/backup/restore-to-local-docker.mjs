...
const [backup, container] = process.argv.slice(2);
if (!backup || !/^supabase_db_af-restore-[a-z0-9-]+$/.test(container ?? "")) {
  console.error("Usage: BACKUP_ENCRYPTION_KEY=<key> node restore-to-local-docker.mjs <backup> supabase_db_af-restore-<name>");
  process.exit(2);
}
